...
class Bubble {
	...
	float _initialRadius = random(128);
	float _radius = this._initialRadius;
	float[] _color = [random(156),random(256),random(256)];
	float _strokeWeight = this._radius / 64;
	PVector _velocity = new PVector((random(1) - 0.5) * 2, (random(1) - 0.5) * 2);
	int _alpha = 0;

	// about children
	int _incidence = 8;
	float _eruptionTime = round(random(32));
	Bubble[] _children = [];
	...
	void update(){
		...
		if(this._updatedCount == this._eruptionTime){
			int nextIncidence = floor(random(this._incidence));
